...
export default function DiskPage() {
  const [diskData, setDiskData] = useState<{ primary: DiskInfo[]; others: DiskInfo[] } | null>(null)
  ...
  return (
    <div className="space-y-6">
      ...
      {diskData && (
        <>
          {/* Primary disks */}
          {diskData.primary.length > 0 && (
            <div>
              <h2 className="text-xl font-semibold mb-2">Primary Disks</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {diskData.primary.map((disk, index) => (
          ...
          {/* Other disks */}
          {diskData.others.length > 0 && (
            <div>
              <h2 className="text-xl font-semibold mt-8 mb-2">Other Devices (loop, swap, etc)</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {diskData.others.map((disk, index) => (
                  <Card key={index} className="hover:shadow-lg transition-shadow">
                    <CardHeader className="pb-3">
                      <CardTitle className="flex items-center space-x-2">
      ...
      {diskData && diskData.primary.length === 0 && diskData.others.length === 0 && (
        <Card>
      ...
      {diskData && (diskData.primary.length > 0 || diskData.others.length > 0) && (
        <>
          {diskData.primary.length > 0 && (
            <Card className="mb-6">
              <CardHeader>
                <CardTitle>Storage Summary: Primary Disks</CardTitle>
                <CardDescription>Overview of all primary mounted filesystems</CardDescription>
              </CardHeader>
              <CardContent>
          ...
          {diskData.others.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle>Storage Summary: Other Devices</CardTitle>
                <CardDescription>Overview of loop, swap, and other virtual/temporary filesystems</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
